...
import {SelectOption, mapSelectValueLabel} from "../../types/SelectOption.jsx";
...
class TagSelectWrapper extends React.PureComponent<Props, State> {
  ...
  initializeSelectedTags(props: Props, displayList: $ReadOnlyArray<SelectOption>): ?(TagDefinition | Array<SelectOption>) {
    if(props.value) {
      const displayTags: $ReadOnlyArray<SelectOption> = props.value[0]
        ? props.value.map(tag => this.getDisplayTag(tag, displayList || this.state.displayList))
        : [null];
  ...
  handleSelection(selectedValueOrValues: TagDefinition | $ReadOnlyArray<TagDefinition>): void {
    ...
    if(selectedValueOrValues) {
      ...
      let tags: TagDefinition = Object.seal(selectedValues.map(value => this.state.tagMap[value.value]));
